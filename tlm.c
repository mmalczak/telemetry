...
int main(int argc, char *argv[])
{
	...
	case 'h':
		...
		// help function terminates application.
		next_arg += 1;
		break;
	case 'p':
		model_params = 1;
	...
	if (devname) {
		...
		if (show_output) {
			// For the case when some trash data are loaded
			// instead of valid tlm,
			// eg ./tlm -d /dev/sda1
			numel = min(tlm.data_count, TLM_BUFFER_SIZE);
			...
			case HR:
				...
				for (i = 0; labelsDef[i]; i++)
				{
					...
					int j;
					if(!strcmp("theta", labelsDef[i]))
						for(j=0; j<deg-1; j++)
							printf("\t");
					if(!strcmp("R", labelsDef[i]))
						for(j=0; j<d_R; j++)
							printf("\t");
					if(!strcmp("S", labelsDef[i]))
						for(j=0; j<d_S; j++)
							printf("\t");
					if(!strcmp("T", labelsDef[i]))
						for(j=0; j<d_T; j++)
							printf("\t");
					if(!strcmp("D", labelsDef[i]))
						for(j=0; j<d_D; j++)
							printf("\t");
				}
				printf("\n");
